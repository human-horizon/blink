...
- [2026-08-28] Проблема: `blink check` не находил Cargo-root в `src/lib.rs` → Решение: добавлен поиск `src/main.rs` и `src/lib.rs`.
- [2026-08-28] Проблема: ошибка lexer превращалась в ложный успешный check → Решение: parser теперь возвращает ошибку lexer.
- [2026-08-28] Проблема: parser зацикливался после ошибки внутри inline-модуля → Решение: вложенный цикл останавливается при первой ошибке.
- [2026-08-28] Проблема: проверка `bevy_ecs` исчерпывала память на тестовом модуле → Решение: `cfg(test)`-модули пропускаются; дальнейшая оптимизация checker ещё нужна.
